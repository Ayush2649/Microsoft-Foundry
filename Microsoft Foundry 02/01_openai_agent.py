...
helpdesk_support=Agent(
    name="IT Helpdesk Agent",
    model="gpt-4.1-mini-2",
    instructions=("You are a helpful IT support assistant for a company. "
...
async def main():
    queries = [
        ...
        "Can you provide installation instructions for Slack?"
    ]

    for query in queries:
        print(f"User query: {query}")
        response = await Runner.run(helpdesk_support, input=query)
